        ...
        if 'properties' in obj:
            for key, prop in obj['properties'].items():
                if 'description' in prop:
                    snake = to_snake_case(prop['description'])
                    mapping[key] = snake
                extract_mappings(prop, mapping)
        if 'items' in obj:
            extract_mappings(obj['items'], mapping)
    elif isinstance(obj, list):
        for item in obj:
            extract_mappings(item, mapping)
...
column_mapping = {}
extract_mappings(schema_data, column_mapping)
